...
type dicomStoreClient interface {
	readResource(storePath string) (*healthcare.DicomStore, error)
	listResources(parent string) ([]*healthcare.DicomStore, error)
...
// Get specific Dicom store by resource URI
//   - storePath: projects/%s/locations/%s/datasets/%s/dicomStores/%s
func (c *dicomStoreClientImpl) readResource(storePath string) (*healthcare.DicomStore, error) {
	return c.dicomStoreService().Get(storePath).Do()
}
